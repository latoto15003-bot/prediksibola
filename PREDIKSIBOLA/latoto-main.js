/* BUILD_VERSION: latoto-prediksi-bola-v1 */
console.log("[bolaauto latoto] Prediksi Bola LATOTO aktif");
(function(){

/* ═══════════════════════════════════════════════
   KONFIGURASI BRAND LATOTO
═══════════════════════════════════════════════ */
var SOURCE_URL    = 'https://shortq.xyz/prediksibola'; // sumber data prediksi cadangan
var SITE_NAME     = 'LATOTO';
var SITE_LOGO     = './latoto-logo.png';
var MBAPPE_GIF    = './giffmbappee-finall.gif';
var MARQUEE_TEXT  = '⚽ PREDIKSI BOLA TERUPDATE ! Tunggu apa lagi? Daftar di LATOTO dan nikmati pengalaman taruhan terbaik! ⚽';
var COLOR_MAIN    = '#c49e1f'; // Emas Elegan LATOTO
var COLOR_TEXT    = '#fffdf0';
var BG_IMAGE      = './latoto-background-web.png';
var BG_OVERLAY    = '#000000, #000000';
var AUTO_REFRESH  = 5 * 60 * 1000; // 5 menit
var CACHE_KEY     = 'latoto_pred_cache_v1';
...
function buildBigMatchHTML(leagues){
  ...
  return '<div class="bm-section">'
    + '<div class="bm-head">'
    +   '<span class="bm-kicker">&#10022; Pertandingan Spesial LATOTO</span>'
    + '</div>'
...
/* ═══════════════════════════════════════════════
   ROOT ELEMENT & LOADING
═══════════════════════════════════════════════ */
var root = document.getElementById('latoto-root') || document.getElementById('mancingduit-root');

(function(){
  var s=document.createElement('style');
  s.textContent=
    '.ibc-loading-wrap{display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:320px;gap:14px;font-family:\'Poppins\',sans-serif;background:#000000;border-radius:12px;padding:30px 20px;}'
    +'.ibc-spinner{width:46px;height:46px;border:4px solid rgba(196,158,31,0.2);border-top-color:#ffd700;border-radius:50%;animation:ibcSpin .7s linear infinite;}'
    +'@keyframes ibcSpin{to{transform:rotate(360deg)}}'
    +'.ibc-loading-text{color:#ffd700;font-size:13px;font-weight:700;letter-spacing:1.5px;text-shadow:0 0 10px rgba(255,215,0,0.5);text-align:center;}'
    +'.ibc-loading-sub{color:rgba(255,215,0,0.5);font-size:10px;letter-spacing:1px;text-align:center;margin-top:-8px;}'
    +'.ibc-progress-wrap{width:80%;max-width:320px;margin-top:2px;}'
    +'.ibc-progress-track{width:100%;height:10px;background:rgba(255,255,255,0.06);border-radius:8px;overflow:hidden;border:1px solid rgba(196,158,31,0.3);position:relative;}'
    +'.ibc-progress-fill{height:100%;width:0%;background:linear-gradient(90deg,#7a5c0b,#c49e1f,#ffd700);border-radius:8px;transition:width .4s ease;box-shadow:0 0 12px rgba(255,215,0,0.4),inset 0 1px 0 rgba(255,255,255,0.25);}'
    +'.ibc-progress-pct{color:#ffd700;font-size:12px;font-weight:700;letter-spacing:1px;text-align:center;margin-top:6px;text-shadow:0 0 8px rgba(255,215,0,0.4);}';
...
function showLoading(msg, pct){
  ...
  root.innerHTML =
    ...
    '<img src="'+SITE_LOGO+'" alt="'+SITE_NAME+'" style="width:140px;max-height:80px;object-fit:contain;filter:drop-shadow(0 0 16px rgba(255,215,0,0.6));margin-bottom:8px;"/>'+
    '<div class="ibc-spinner"></div>'+
    '<div class="ibc-loading-text">⚽ '+(msg||'Memuat Prediksi LATOTO…')+'</div>'+
    (hasBar?
...
async function fetchHTML(url){
  ...
  for(var i=0;i<totalProxies;i++){
    ...
    try{
      ...
      console.log('['+SITE_NAME+'] Mencoba '+px.name+'…');
      ...
      console.log('['+SITE_NAME+'] OK via '+px.name);
      updateProgress(60);
      return html;
    }catch(e){
      console.warn('['+SITE_NAME+'] '+px.name+' gagal:',e.message);
    }
  }
...
function buatPasaran(m){
  ...
  if(ada && s2>s1) return nilai+' : 0';
  if(ada && s1>s2) return '0 : '+nilai;
  return (seed & 1) ? nilai+' : 0' : '0 : '+nilai;
}
...
var FLAG_MAP={
  ...
  'new zealand':'nz','bosnia-herzegovina':'ba','paraguay':'py'
};
...
function makeSVG(name){
  ...
  var cols=['#FFD700','#C49E1F','#E27A03','#FFB800','#D4AF37','#E5A912','#F39C12'];
...
function buildDateRange(leagues){
  ...
  var dMaxAllowed=new Date(toDate(first).getTime()+86400000);
  var last=first;
...
/* ═══════════════════════════════════════════════
   BUILD OUTPUT HTML (GOLD LATOTO THEME)
═══════════════════════════════════════════════ */
...
function buildOutputHTML(leagues){
  ...
  var bigMatchSec = buildBigMatchHTML(leagues);

  return '<!-- Generated by LATOTO Auto Embed -->\n'
+'<style>\n'
+'*,*::before,*::after{box-sizing:border-box;margin:0;padding:0;}\n'
+':root{--g:'+bg+';--gl:'+ltr+';--g2:'+drk+';--gd:'+bg+'88;--gs:'+bg+'2a;--cr:'+txt+';--bg:#000000;--bg2:#0d0d0d;}\n'
...
+'#latoto-root, #mancingduit-root{background:linear-gradient(135deg,'+BG_OVERLAY+'),url("'+BG_IMAGE+'") center/cover fixed no-repeat #000;padding:1px 0;border-radius:0;min-height:100px;}\n'
...
+'.date-display{display:flex;align-items:center;justify-content:space-between;margin:14px auto;width:90%;padding:8px 10px;border:3px solid var(--g);border-radius:15px;background:linear-gradient(180deg,#1f1807,#000);box-shadow:0 0 20px var(--gd);overflow:hidden;}\n'
+'.date-text{flex:1;text-align:center;font-family:\'Cinzel\',serif;font-size:clamp(11px,2.8vw,16px);font-weight:700;color:#ffd700;text-shadow:0 0 14px var(--gd);letter-spacing:2px;word-break:break-word;padding:0 6px;}\n'
+'.stats-bar{width:90%;margin:0 auto 14px;display:grid;grid-template-columns:repeat(3,1fr);gap:8px;}\n'
+'.stat-item{background:linear-gradient(180deg,#1f1807,#0a0800);border:2px solid var(--gd);border-radius:13px;padding:11px 6px;text-align:center;}\n'
+'.stat-num{font-family:\'Cinzel\',serif;font-size:20px;font-weight:900;color:#ffd700;line-height:1;display:block;}\n'
+'.stat-lbl{font-size:9px;font-weight:700;color:var(--g);opacity:.85;letter-spacing:1.2px;text-transform:uppercase;display:block;margin-top:3px;}\n'
+'.marquee-wrap{overflow:hidden;white-space:nowrap;width:90%;margin:0 auto 16px;border:3px solid var(--g);border-radius:15px;background:linear-gradient(135deg,#1f1807,#000,#1f1807);padding:15px 0;box-shadow:0 0 20px var(--gd);}\n'
+'.marquee-inner{display:inline-block;animation:marquee 38s linear infinite;padding-left:100%;font-size:clamp(11px,2.8vw,14px);font-weight:900;color:#ffd700;text-shadow:0 0 10px var(--gd);letter-spacing:1.5px;}\n'
+'@keyframes marquee{0%{transform:translateX(0)}100%{transform:translateX(-100%)}}\n'
+'.bm-section{width:90%;margin:0 auto 18px;}\n'
+'.bm-head{margin-bottom:14px;text-align:center;}\n'
+'.bm-kicker{display:inline-flex;align-items:center;gap:8px;font-family:\'Cinzel\',serif;font-size:clamp(12px,3vw,16px);font-weight:900;color:#ffd700;text-shadow:0 0 12px var(--gd);letter-spacing:1.5px;padding:6px 16px;border:2px solid var(--g);border-radius:12px;background:linear-gradient(135deg,#241a00,#000);}\n'
...
+'  background:linear-gradient(180deg,rgba(20,20,20,.6),rgba(10,10,10,.8)),url("https://cdn.areabermain.club/assets/cdn/az5/2026/08/10/20260810/a9f1f7ce6f0aad4e3eda5bd9c0b9610e/a1da402c-b9ab-4238-8e0f-50d160a2291b.png") center/cover no-repeat,#1a1300;'
...
+'.bm-league{text-align:center;font-size:9.5px;font-weight:800;letter-spacing:1.5px;color:#ffd700;text-transform:uppercase;margin-bottom:12px;line-height:1.3;text-shadow:0 0 8px var(--gd);}\n'
...
+'.bm-teams em{font-style:normal;font-weight:900;font-size:12px;color:#ffd700;opacity:.8;letter-spacing:1px;}\n'
+'.bm-meta{text-align:center;font-size:10.5px;font-weight:700;color:#ffd700;opacity:.9;letter-spacing:.5px;margin-bottom:12px;}\n'
...
+'.bm-val i{font-style:normal;font-size:9px;font-weight:700;color:#ffd700;opacity:.7;letter-spacing:1.5px;text-transform:uppercase;}\n'
+'.bm-val b{font-size:18px;font-weight:900;color:#ffd700;text-shadow:0 0 10px var(--gd);}\n'
+'.filter-wrap{width:90%;margin:0 auto 14px;display:grid;grid-template-columns:1fr 1fr;gap:10px;}\n'
+'.filter-label{font-size:10px;color:#ffd700;opacity:.85;text-transform:uppercase;letter-spacing:1.5px;font-weight:700;margin-bottom:6px;display:block;}\n'
+'.select-box{position:relative;}\n'
+'.select-box::after{content:"\\25BC";position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#ffd700;font-size:10px;pointer-events:none;}\n'
+'.fselect{width:100%!important;height:38px!important;padding:0 30px 0 12px!important;background:rgba(0,0,0,.85)!important;border:2px solid var(--gd)!important;border-radius:10px!important;color:#ffd700!important;font-size:12px!important;font-weight:700!important;appearance:none!important;-webkit-appearance:none!important;outline:none!important;cursor:pointer;line-height:34px!important;box-shadow:none!important;box-sizing:border-box!important;}\n'
...
+'.fsearch{width:100%!important;height:38px!important;padding:0 12px 0 28px!important;background:rgba(0,0,0,.85)!important;border:2px solid var(--gd)!important;border-radius:10px!important;color:#ffd700!important;font-size:12px!important;font-weight:700!important;outline:none!important;font-family:\'Poppins\',sans-serif!important;line-height:34px!important;box-shadow:none!important;-webkit-appearance:none!important;appearance:none!important;box-sizing:border-box!important;}\n'
+'.fsearch::placeholder{color:rgba(255,215,0,0.5)!important;font-weight:600!important;opacity:1!important;}\n'
+'.fsearch:focus{border-color:#ffd700!important;background:rgba(0,0,0,.85)!important;}\n'
...
+'.hint-text{font-family:\'Cinzel\',serif;font-size:clamp(9px,2.2vw,11px);font-weight:700;color:#ffd700;opacity:.8;letter-spacing:1px;}\n'
...
+'.league-crown{background:linear-gradient(180deg,#241802,#0d0d0d);padding:8px 10px 6px;display:flex;align-items:center;justify-content:center;position:relative;border-bottom:1px solid var(--gs);overflow:hidden;}\n'
...
+'@keyframes crownshine{0%{left:-120%}100%{left:200%}}\n'
+'.crown-name{font-family:\'Cinzel\',serif;font-size:clamp(11px,2.8vw,14px);font-weight:900;color:#ffd700;text-shadow:0 0 10px var(--gd);letter-spacing:1.5px;text-align:center;line-height:1.2;}\n'
+'.match-count{position:absolute;right:8px;top:50%;transform:translateY(-50%);font-size:9px;font-weight:700;letter-spacing:1px;color:#000;background:linear-gradient(180deg,#ffd700,#c49e1f);padding:2px 8px;border-radius:10px;white-space:nowrap;}\n'
+'.match-card{border-bottom:1px solid var(--gs);cursor:pointer;background:linear-gradient(90deg,transparent,rgba(196,158,31,0.03),transparent);transition:background .2s;}\n'
+'.match-card:last-child{border-bottom:none;}\n'
+'.match-card:hover{background:rgba(196,158,31,0.08);}\n'
+'.match-card.hidden{display:none;}\n'
+'.match-row{display:grid;grid-template-columns:1fr auto 1fr auto;align-items:center;padding:10px 10px;gap:6px;position:relative;}\n'
+'.team-side{display:flex;align-items:center;gap:7px;min-width:0;}\n'
+'.team-side.right{flex-direction:row-reverse;text-align:right;}\n'
+'.box-image{width:32px;height:32px;flex-shrink:0;position:relative;display:flex;align-items:center;justify-content:center;}\n'
+'.team-logo{width:100%;height:100%;object-fit:contain;filter:drop-shadow(0 0 4px var(--gd));position:relative;z-index:1;}\n'
+'.character{position:absolute;width:100%;height:100%;object-fit:contain;filter:blur(8px) opacity(.4);top:0;left:0;}\n'
+'.team-name{font-size:clamp(10px,2.4vw,12px);font-weight:700;color:#fff;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;line-height:1.2;}\n'
+'.score-center{display:flex;flex-direction:column;align-items:center;justify-content:center;min-width:68px;padding:0 4px;}\n'
+'.score-num{font-family:\'Cinzel\',serif;font-size:clamp(13px,3.2vw,17px);font-weight:900;color:#ffd700;letter-spacing:2px;text-shadow:0 0 10px var(--gd);line-height:1;white-space:nowrap;}\n'
+'.match-dt{font-size:9px;color:#ffd700;opacity:.8;letter-spacing:.5px;margin-top:3px;text-align:center;line-height:1.3;white-space:nowrap;}\n'
+'.chev{display:flex;align-items:center;justify-content:center;color:var(--g);transition:transform .3s ease;flex-shrink:0;padding-left:2px;}\n'
+'.match-card.open .chev{transform:rotate(180deg);}\n'
+'.pred-panel{display:none;padding:0 10px 10px;}\n'
+'.match-card.open .pred-panel{display:block;}\n'
+'.pred-inner{background:rgba(0,0,0,.7);border:1px solid var(--gd);border-radius:10px;padding:10px;}\n'
...
+'.pred-col{display:flex;flex-direction:column;align-items:center;text-align:center;padding:7px 4px;background:rgba(255,255,255,.03);border-radius:8px;border:1px solid transparent;}\n'
+'.pred-col.accent-green{border-color:rgba(0,230,118,.35);}\n'
+'.pred-col.accent-red{border-color:rgba(255,82,82,.35);}\n'
+'.pred-col.accent-gold{border-color:rgba(255,215,0,.35);}\n'
+'.col-title{font-size:8.5px;color:rgba(255,255,255,.6);text-transform:uppercase;letter-spacing:1px;font-weight:700;margin-bottom:4px;}\n'
+'.col-answer{font-size:12px;font-weight:900;color:#ffd700;line-height:1.2;word-break:break-word;}\n'
+'.col-answer.green{color:#00e676;}\n'
+'.col-answer.red{color:#ff5252;}\n'
+'.col-note{font-size:8px;color:rgba(255,255,255,.45);margin-top:3px;letter-spacing:.5px;}\n'
+'</style>\n'
+'<div class="site-logo">\n'
+'  <img class="mbappe-gif" src="'+MBAPPE_GIF+'" alt="LATOTO" loading="lazy" onclick="this.classList.toggle(\'flip\')" title="Klik untuk membalik">\n'
+'  <img class="logo-main" src="'+SITE_LOGO+'" alt="LATOTO" loading="lazy">\n'
+'  <img class="mbappe-gif flip" src="'+MBAPPE_GIF+'" alt="LATOTO" loading="lazy" onclick="this.classList.toggle(\'flip\')" title="Klik untuk membalik">\n'
+'</div>\n'
+'<div class="date-display">\n'
+'  <div class="date-text">&#9917; '+autoDate+' &#9917;</div>\n'
+'</div>\n'
+'<div class="stats-bar">\n'
+'  <div class="stat-item"><span class="stat-num">'+totalMatches+'</span><span class="stat-lbl">Pertandingan</span></div>\n'
+'  <div class="stat-item"><span class="stat-num">'+totalLeagues+'</span><span class="stat-lbl">Kompetisi</span></div>\n'
+'  <div class="stat-item"><span class="stat-num">98%</span><span class="stat-lbl">Akurasi</span></div>\n'
+'</div>\n'
+'<div class="marquee-wrap">\n'
+'  <div class="marquee-inner">'+MARQUEE_TEXT+'</div>\n'
+'</div>\n'
+bigMatchSec
+'<div class="filter-wrap">\n'
+'  <div class="filter-group">\n'
+'    <label class="filter-label">&#127942; Pilih Liga</label>\n'
+'    <div class="select-box">\n'
+'      <select class="fselect" id="leagueFilter" onchange="ibcFilterLeague(this.value)">\n'
+'        <option value="all">Semua Liga ('+totalLeagues+')</option>\n'
+leagueOpts
+'      </select>\n'
+'    </div>\n'
+'  </div>\n'
+'  <div class="filter-group">\n'
+'    <label class="filter-label">&#128269; Cari Tim</label>\n'
+'    <div class="search-box">\n'
+'      <span class="search-icon">&#128269;</span>\n'
+'      <input class="fsearch" id="teamSearch" type="text" placeholder="Ketik nama tim…" oninput="ibcSearchTeam(this.value)"/>\n'
+'    </div>\n'
+'  </div>\n'
+'</div>\n'
+'<div class="tap-hint"><span class="hint-arrow">&#9660;</span><span class="hint-text">Klik Pada Pertandingan Untuk Melihat Rincian Prediksi</span><span class="hint-arrow">&#9660;</span></div>\n'
+blocks
+'<script>\n'
...
+'  var s=document.getElementById("teamSearch"); if(s)s.value="";\n'
...
+'  var f=document.getElementById("leagueFilter"); if(f)f.value="all";\n'
...
+'  var visible=true, last=0, SPEED=32;\n'
...
function injectHTML(html){
  if(!root) root = document.getElementById('latoto-root') || document.getElementById('mancingduit-root');
  if(!root) return;
  root.innerHTML = html;
...
async function fetchFreshOutput(showProgress){
  ...
  if (JPK_ENABLED) {
    ...
    for (var ju = 0; ju < jpkUrls.length && !lines; ju++) {
      try {
        console.log('['+SITE_NAME+'] Coba jpkoloni: ' + jpkUrls[ju]);
        var jpkHtml = await fetchHTML(jpkUrls[ju]);
        var jpkLines = parseJpkoloni(jpkHtml);
        if (jpkLines.length) {
          lines = jpkLines;
          console.log('['+SITE_NAME+'] jpkoloni OK (' + jpkLines.length + ' baris)');
        }
      } catch (eJpk) { console.warn('['+SITE_NAME+'] jpkoloni gagal: ' + eJpk.message); }
    }
  }
...
async function loadAndRender(){
  ...
  if(cache && cache.html){
    ...
    try {
      ...
      writeCache(freshBg);
      injectHTML(freshBg);
    } catch(e){ console.warn('['+SITE_NAME+'] refresh latar gagal:', e.message); }
    return;
  }

  try {
    showLoading('Memuat Prediksi LATOTO…', 0);
    var output = await fetchFreshOutput(true);
    writeCache(output);
    injectHTML(output);
  } catch(e) {
    console.error('['+SITE_NAME+' Embed]',e);
...
async function backgroundRefresh(){
  try {
    ...
    writeCache(fresh);
    injectHTML(fresh);
  } catch(e){ console.warn('['+SITE_NAME+'] auto-refresh gagal:', e.message); }
}

function init(){
  root = document.getElementById('latoto-root') || document.getElementById('mancingduit-root');
  loadAndRender();
